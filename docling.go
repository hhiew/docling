...
// 作为 docparse 通用组件的"详细 JSON"输出协议：
...
func (r *RefItem) UnmarshalJSON(data []byte) error {
	...
	if !ok {
		return fmt.Errorf("docparse: 非法 $ref: %q", raw.Ref)
	}
	r.Kind = docRefKind(kind)
	var n int64
	if _, err := fmt.Sscanf(idx, "%d", &n); err != nil {
		return fmt.Errorf("docparse: 非法 $ref 下标: %q", raw.Ref)
	}
	r.Idx = n
	return nil
}
...
func (o *DocumentOrigin) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(payload.BinaryHash, &hash); err != nil {
		...
		if stringErr := json.Unmarshal(payload.BinaryHash, &encoded); stringErr != nil {
			return fmt.Errorf("docparse: 非法 binary_hash: %w", err)
		}
		...
		if parseErr != nil {
			return fmt.Errorf("docparse: 非法十六进制 binary_hash: %w", parseErr)
		}
		hash = parsed
	}
